...
# Define all the question lists (as provided in your original code)
malware_questions = [
    {
        "id": 101,
...
# Combine all question lists into a single list
all_questions = (
...
def get_unique_languages_from_questions(questions):
    """
    Extracts all unique programming languages required for a given list of questions.
    Args:
        questions (list): A list of question dictionaries.
    Returns:
        set: A set of unique programming languages.
    """
    languages = set()
    for question in questions:
        if "language_required" in question and question["language_required"] != "N/A":
            languages.add(question["language_required"])
    return languages
...
def score_quiz(questions, user_answers):
    ...
    Args:
        questions (list): The list of questions presented in the quiz.
        user_answers (dict): A dictionary of user's answers, mapping question ID to chosen option.
    Returns:
        tuple: A tuple containing the total score and a dictionary of field-wise scores.
    """
    score = 0
    field_scores = {}
    for q in questions:
...
def recommend_field(field_scores):
    """
    Recommends a field based on the field-wise scores.
    Args:
        field_scores (dict): A dictionary of field-wise scores.
    Returns:
        str: A recommended field or "No strong recommendation".
    """
    if not field_scores:
...
def main():
    ...
    st.title("🛡️ Cybersecurity Quiz!")
    st.write("Test your knowledge across various cybersecurity domains.")
    ...
    if 'quiz_started' not in st.session_state:
        ...
        st.session_state.field_scores = {}
        st.session_state.languages_covered = set()

    if not st.session_state.quiz_started:
        st.header("Choose Your Quiz Category")

        unique_fields = sorted(list(set(q['field'] for q in all_questions)))
        category_options = ["All Categories"] + unique_fields
        selected_category = st.selectbox(
            "Select a category to quiz on:",
            category_options
        )

        if st.button("Start Quiz"):
            if selected_category == "All Categories":
                st.session_state.selected_questions = random.sample(all_questions, min(len(all_questions), 20)) # Limit to 20 questions for 'All'
            else:
                st.session_state.selected_questions = [q for q in all_questions if q['field'] == selected_category]
            ...
            st.session_state.current_question_index = 0
            st.session_state.user_answers = {}
            st.rerun() # Rerun to start the quiz display
        ...
        if current_q_idx < len(questions):
            ...
            st.subheader(f"Question {current_q_idx + 1}/{len(questions)} ({question_data['field']})")
            st.markdown(f"**Question ID:** {question_data['id']}")
            st.markdown(f"**Language Required:** {question_data['language_required']}")
            ...
            user_choice = st.radio("Select your answer:", options, key=f"q_{question_data['id']}")

            if st.button("Submit Answer", key=f"submit_{question_data['id']}"):
                st.session_state.user_answers[question_data['id']] = user_choice
            ...
            st.session_state.total_score, st.session_state.field_scores = score_quiz(
                st.session_state.selected_questions, st.session_state.user_answers
            )
            st.session_state.languages_covered = get_unique_languages_from_questions(
                st.session_state.selected_questions
            )
        ...
        st.success(f"You answered **{st.session_state.total_score}** out of **{len(st.session_state.selected_questions)}** questions correctly.")

        st.subheader("Field-wise Scores:")
        if st.session_state.field_scores:
            for field, score in st.session_state.field_scores.items():
                st.write(f"- **{field}**: {score} correct")
        else:
            st.write("No field-wise scores to display.")

        recommended = recommend_field(st.session_state.field_scores)
        st.subheader("Based on your answers, we recommend:")
        st.info(f"**{recommended}**")

        if st.session_state.languages_covered:
            st.subheader("Languages Covered in this Quiz:")
            st.write(", ".join(sorted(st.session_state.languages_covered)))
        else:
            st.write("No specific programming languages were highlighted in this quiz category.")
